...
class TestLanguageStatus extends LanguageStatusNotifier {
  ...
  TestLanguageStatus({
    ...
    List<String> langWithUpdates = const [],
...
Client mockCheckResponse(Map<String, int> languageUpdateMap) {
  return MockClient((request) async {
    ...
    final String languageCode = request.url.pathSegments
        .firstWhere(
          (element) => element.startsWith('html-'),
          orElse: () => 'html-xyz',
        )
        .substring(5);
...
void main() {
  ...
  testWidgets('Test getting localized messages', (WidgetTester tester) async {
    ...
    expect(
      CheckFrequency.getLocalized(context, CheckFrequency.never),
      "niemals",
    );
  });
  ...
  test('Test TestLanguageStatusNotifier class', () async {
    ...
    expect(deStatus.updatesAvailable, false);
    expect(
      deStatus.lastCheckedTimestamp.compareTo(deStatus.downloadTimestamp),
      greaterThan(0),
    );
    ...
    expect(
      deStatus3.lastCheckedTimestamp,
      equals(deStatus2.lastCheckedTimestamp),
    );
    ...
    expect(
      deStatus4.lastCheckedTimestamp.compareTo(deStatus3.lastCheckedTimestamp),
      greaterThan(0),
    );
  });
  test(
    'Test TestLanguageStatusNotifier constructor parameter checkReturnValue',
    () async {
      final ref = ProviderContainer(
        overrides: [
          languageStatusProvider.overrideWith2(
            (langCode) => TestLanguageStatus(checkReturnValue: 2),
          ),
        ],
      );
  ...
  test(
    'Test TestLanguageStatusNotifier constructor parameter langsWithUpdates',
    () async {
      var ref = ProviderContainer(
        overrides: [
          languageStatusProvider.overrideWith2(
            (langCode) => TestLanguageStatus(langWithUpdates: ['de']),
          ),
        ],
      );
      expect(ref.read(languageStatusProvider('de')).updatesAvailable, true);
  ...
  test('Test checking for updates: no updates', () async {
    ...
    expect(
      ref.read(lastCheckedProvider),
      equals(deStatus.lastCheckedTimestamp),
    );
    expect(deStatus.updatesAvailable, false);
    expect(prefs.getBool('updatesAvailable-de'), false);
    expect(
      prefs.getString('lastChecked-de'),
  ...
  test('Test updatesAvailableProvider', () async {
    ...
    expect(await ref.read(languageProvider('de').notifier).download(), true);
