...
void main() {
  testWidgets('Test when no updates are available', (
    ...
    final ref = ProviderContainer(
      overrides: [
        ...
        languageStatusProvider.overrideWith2(
          (languageCode) => TestLanguageStatus(checkReturnValue: 0),
        ),
      ],
    );
    ...
    await tester.pumpWidget(
      UncontrolledProviderScope(
        container: ref,
        child: const TestCheckNowButton(),
      ),
    );
  ...
  testWidgets('Test that we ran into Github API rate limit', (
    ...
    // snackbar correct?
    expect(
      find.text(AppLocalizationsDe().checkingUpdatesLimit),
      findsOneWidget,
    );
  ...
  testWidgets('Test unknown error while checking for updates', (
    ...
    final ref = ProviderContainer(
      overrides: [
        ...
        languageStatusProvider.overrideWith2(
          (languageCode) => TestLanguageStatus(checkReturnValue: -1),
        ),
      ],
    );
    ...
    // snackbar correct?
    expect(
      find.text(AppLocalizationsDe().checkingUpdatesError),
      findsOneWidget,
    );
    expect(countCheckCalled, countAvailableLanguages);
  });
